...
async function start(file) {
  ...
  const opcion = await question(chalk.yellowBright.bold('—◉ㅤselect:\n') + chalk.white.bold('1. QR\n2. code\n—> '));

  let numeroTelefono = '';
  if (opcion === '2') {
    const phoneNumber = await question(chalk.yellowBright.bold('\n—◉ㅤnumber:\n') + chalk.white.bold('example: +94772601056\n—> '));
    numeroTelefono = formatearNumeroTelefono(phoneNumber);
    if (!esNumeroValido(numeroTelefono)) {
      console.log(chalk.bgRed(chalk.white.bold('wrong number')));
      process.exit(0);
    }
